...
import com.example.reto3.Vista.FragmentFavoritos;
import com.example.reto3.Vista.FragmentInicio;
import com.example.reto3.Vista.FragmentProductos;
import com.example.reto3.Vista.FragmentServicios;
import com.example.reto3.Vista.FragmentSucursales;
...
public class MainActivity extends AppCompatActivity {
    ...
    public boolean onOptionsItemSelected(@NonNull MenuItem item) {
        ...
        if (id == R.id.productos) {
            intercambio = getSupportFragmentManager().beginTransaction();
            intercambio.replace(R.id.contenedor, pantproductos).addToBackStack(null).addToBackStack(null).commit();
        }
        if (id == R.id.servicios) {
            intercambio = getSupportFragmentManager().beginTransaction();
            intercambio.replace(R.id.contenedor, pantservicios).addToBackStack(null).addToBackStack(null).commit();
        }
        if (id == R.id.sucursales) {
            intercambio = getSupportFragmentManager().beginTransaction();
            intercambio.replace(R.id.contenedor, pantsucursales).addToBackStack(null).addToBackStack(null).commit();
        }
        if (id == R.id.favoritos) {
            intercambio = getSupportFragmentManager().beginTransaction();
            intercambio.replace(R.id.contenedor, pantfavoritos).addToBackStack(null).addToBackStack(null).commit();
        }
